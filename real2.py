...
for ts in range(st, ed):
    if ts > detect_st-24:
        ...
        tmpX = np.c_[tmpX[:, -nS * nNearPart:], tmpX[:, :-nS * nNearPart].reshape((nR, t_delta - 1, nNearPart, nS))[:, :, 0,
                                                :].reshape((nR, -1))]
        # tmpX=862*12
    ...
    # 时间点到到检测时间之后
    if ts > detect_st:
        if  not trained:#ts % (60 // MPS * 24) == 0 or
            #model_r.fit(train_r)
            #model_int.fit(train_int)
            trained = True
            model_r = joblib.load('模型/model_r.pkl')
            model_int = joblib.load('模型/model_int.pkl')
        ...
        #for element in anomalies[ts]:
          #  if element==1:
           #     num+=1
print(num)
current_time=(datetime.now())
formatted_time = current_time.strftime("%Y-%m-%d-%H-%M-%S")
np.savetxt(dataRoot4 + formatted_time+'_'+"anomalies3.txt", anomalies)  # detected anomalies